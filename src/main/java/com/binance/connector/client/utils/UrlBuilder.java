...
public final class UrlBuilder {
	...
	public static String buildFullUrl(String baseUrl, String urlPath, LinkedHashMap<String, Object> parameters,
			String signature) {
	...
	public static StringBuilder joinQueryParameters(StringBuilder urlPath, LinkedHashMap<String, Object> parameters) {
		...
		for (Map.Entry<String, Object> mapElement : parameters.entrySet()) {
			...
			urlPath.append(mapElement.getKey()).append('=').append(urlEncode(mapElement.getValue().toString()));
		}
		return urlPath;
	}

	private static void joinArrayListParameters(String key, StringBuilder urlPath, ArrayList<?> values,
			boolean isFirst) {
		for (Object value : values) {
			...
			urlPath.append(key).append('=').append(urlEncode(value.toString()));
		}
	}
